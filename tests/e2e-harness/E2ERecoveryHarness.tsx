...
const OP_STORAGE_ID = "beatgaler-e2e-phase11-operation";
const INDEX_STORAGE_ID = "beatgaler-e2e-phase11-index";
const TMP_STORAGE_ID = "beatgaler-e2e-phase11-temp";

function readOperation(): DurableOperation | null {
  try {
    const raw = localStorage.getItem(OP_STORAGE_ID);
...
export default function E2ERecoveryHarness() {
  ...
  const [canonicalIndex, setCanonicalIndex] = useState(() => localStorage.getItem(INDEX_STORAGE_ID) || "index-v1");
  const [tempPresent, setTempPresent] = useState(() => localStorage.getItem(TMP_STORAGE_ID) === "1");
  const [corruptAccepted, setCorruptAccepted] = useState(false);

  useEffect(() => {
    if (!localStorage.getItem(INDEX_STORAGE_ID)) localStorage.setItem(INDEX_STORAGE_ID, "index-v1");
  }, []);
  ...
  const start = (kind: OperationKind) => {
    ...
    localStorage.setItem(OP_STORAGE_ID, JSON.stringify(next));
    localStorage.setItem(TMP_STORAGE_ID, "1");
    setOperation(next);
    setTempPresent(true);
    setState("running");
  };
  ...
  const complete = () => {
    ...
    localStorage.removeItem(OP_STORAGE_ID);
    localStorage.removeItem(TMP_STORAGE_ID);
    setOperation(null);
  ...
  const simulateTornIndexCommit = () => {
    ...
    localStorage.setItem(`${INDEX_STORAGE_ID}-candidate`, "TRUNCATED-CANDIDATE");
    setCanonicalIndex(localStorage.getItem(INDEX_STORAGE_ID) || "index-v1");
  };

  const recoverIndex = () => {
    localStorage.removeItem(`${INDEX_STORAGE_ID}-candidate`);
    setCanonicalIndex(localStorage.getItem(INDEX_STORAGE_ID) || "index-v1");
  };
  ...
  return (
    <main data-e2e-recovery-harness="true" data-e2e-state={state} data-e2e-operation={status}
      ...
      <button data-e2e-reset onClick={() => {
        localStorage.removeItem(OP_STORAGE_ID); localStorage.removeItem(`${INDEX_STORAGE_ID}-candidate`); localStorage.removeItem(TMP_STORAGE_ID);
        localStorage.setItem(INDEX_STORAGE_ID, "index-v1"); setOperation(null); setState("idle"); setCompletedIds([]); setCanonicalIndex("index-v1"); setTempPresent(false); setCorruptAccepted(false);
      }}>Reset</button>
      <section>
