...
function storageChatId(account) {
  ...
  if (!Number.isFinite(value) || value === 0) {
    throw new Error("BeatGaler Storage is not configured. Send /beatgaler_storage inside the private Topics supergroup.");
  }
  return value;
}
...
async function ensureBeatTopic(account, userId, beatId, beatName) {
  ...
  if (!Number.isFinite(messageThreadId) || messageThreadId <= 0) {
    throw new Error("Telegram returned no message_thread_id.");
  }
...
async function deleteBeatTopic(account, userId, beatId, topicIdHint) {
  ...
  try {
    await telegramJsonMethod("deleteForumTopic", {
      chat_id: storageChatId(account),
      message_thread_id: messageThreadId,
    });
  } catch (error) {
    const message = String(error?.message || error);
    if (!/message thread not found|topic.*not found|MESSAGE_THREAD_INVALID/i.test(message)) throw error;
  }
...
async function flushPendingTopicDeletes(account, userId) {
  ...
  for (const [key, entry] of [...pendingTopicDeletes.entries()]) {
    if (!key.startsWith(prefix)) continue;
    try {
      await deleteBeatTopic(account, userId, entry.beatId, entry.telegramTopicId);
    } catch (error) {
      console.warn("[topics] permanent delete failed; kept for retry:", error?.message || error);
    }
  }
}
...
async function readLibraryManifestFromChat(chatId) {
  try {
    const chat = await bot.getChat(Number(chatId));
...
async function writeMigratedLibraryIndex(account, manifest) {
  if (!manifest || !Array.isArray(manifest.beats)) return;
  const chatId = storageChatId(account);
  ...
  try {
    ...
    if (existing) {
      sent = await editTelegramDocumentInPlace({
        ...
        filePath: tempPath,
        filename: "beatgaler-library.json",
...
async function getPinnedLibraryIndex(account) {
  const chat = await bot.getChat(storageChatId(account));
  const pinned = chat?.pinned_message;
...
app.post("/library/upsert", upload.single("file"), async (req, res) => {
  ...
  try {
    const incomingRaw = await fs.promises.readFile(req.file.path, "utf8");
    const incomingManifest = JSON.parse(incomingRaw);
    injectTopicIds(beatgalerUserId, incomingManifest);
    await fs.promises.writeFile(req.file.path, JSON.stringify(incomingManifest, null, 2), "utf8");

    const existing = await getPinnedLibraryIndex(account);
    let previousManifest = null;
    if (existing?.document?.file_id) {
      try {
        const raw = await downloadTelegramFileBuffer(existing.document.file_id);
        previousManifest = JSON.parse(raw.toString("utf8"));
      } catch (error) {
        console.warn("[library] previous index unreadable:", error?.message || error);
      }
    }
    let sent;
    let updated = false;
    ...
    queueRemovedTrashTopics(beatgalerUserId, previousManifest, incomingManifest);
    await flushPendingTopicDeletes(account, beatgalerUserId);
    cleanup();
...
bot.onText(/\/beatgaler_storage(?:@\w+)?/, async (msg) => {
  ...
  try {
    ...
    const previousPrivateChatId = account.telegramUserId;

    account.storageChatId = chatId;
    account.storageChatTitle = chat.title || "BeatGaler Storage";
    account.storageLinkedAt = Date.now();
    linkedAccounts.set(userId, account);
    savePersistentData();

    const migration = await migrateCurrentLibraryToTopics(account, userId, previousPrivateChatId);

    broadcastCloudEvent(userId, "telegram_storage_connected", {
      storage_chat_id: String(chatId),
      storage_chat_title: account.storageChatTitle,
      migrated_topics: migration.migrated,
    });
