...
fn beat_from_cloud_manifest_entry(
    ...
    let has_project = entry.get("project").map(|v| !v.is_null()).unwrap_or(false);
    ...
    Ok(BeatMeta {
        ...
        has_samples: existing_local.as_ref().map(|b| b.has_samples).unwrap_or(false) || has_project,
        samples_path: existing_local.as_ref().and_then(|b| b.samples_path.clone()),
        has_flp: existing_local.as_ref().map(|b| b.has_flp).unwrap_or(false) || has_project,
