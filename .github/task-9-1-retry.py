...
replace_once(
    ...
    "folder preference prop",
)
app_path.write_text(text)

Path("src/features/session").mkdir(parents=True, exist_ok=True)
Path("tests/component-dom").mkdir(parents=True, exist_ok=True)
Path("tests/integration").mkdir(parents=True, exist_ok=True)

Path("src/features/session/useSessionState.ts").write_text(r'''import { useState } from "react";
...
Path("src/features/session/useCustomCursor.ts").write_text(r'''import { useEffect } from "react";
...
Path("src/features/session/useSessionActions.ts").write_text(r'''import { useCallback } from "react";
import type { Dispatch, SetStateAction } from "react";
...
Path("tests/component-dom/sessionState.test.tsx").write_text(r'''// @vitest-environment jsdom
...
Path("tests/integration/appSessionExtraction.test.ts").write_text(r'''import fs from "node:fs";
