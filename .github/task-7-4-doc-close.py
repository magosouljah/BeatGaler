...
IMPLEMENTATION_SHA = "3c485083e4fe6e896017e2e26f4a33d9e25902b5"

roadmap_path = Path("migration/BeatGaler-roadmap-para-trabajar-con-IAs.md")
roadmap = roadmap_path.read_text(encoding="utf-8")
old = "### [ ] 7.4 — Separar la entrada de importación web"
new = "### [x] 7.4 — Separar la entrada de importación web"
if new not in roadmap:
    if old not in roadmap:
        raise SystemExit("7.4 roadmap marker not found")
...
```
'''
if "### Registro — 7.4" not in registro:
    registro = registro.rstrip() + "\n\n" + entry.strip() + "\n"
    registro_path.write_text(registro, encoding="utf-8")

close_run = os.environ.get("GITHUB_RUN_ID", "unknown")
agent = f'''# BeatGaler — Agent State
...
- Run de recuperación documental: `{close_run}` — el árbol final de recuperación modifica solo los tres documentos de migración respecto al SHA validado.
...
- Las pruebas focalizadas y la matriz completa de checks pasaron sobre `{IMPLEMENTATION_SHA}`.
...
'''
Path("migration/BeatGaler-agent-state.md").write_text(agent, encoding="utf-8")
