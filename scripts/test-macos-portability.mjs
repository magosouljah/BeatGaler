...
const fastWorkflow = read(".github/workflows/build-macos-fast.yml");
const releaseWorkflow = read(".github/workflows/build-macos-cloud-beta.yml");
const windowsUpdater = read(".github/workflows/release-windows-updater.yml");
...
const portabilityWorkflow = read(".github/workflows/test-macos-portability.yml");
...
ok(!exists(".github/workflows/build-macos.yml"), "obsolete macOS workflow cannot emit a structurally different DMG");
const pinnedCommit = "adfd7f6a8e990272851777eeb3ae0def4216f161";
ok(fastWorkflow.includes(pinnedCommit) && releaseWorkflow.includes(pinnedCommit), "Bot API source is pinned to an exact commit in both Mac pipelines");
ok(fastWorkflow.includes('MACOSX_DEPLOYMENT_TARGET: "12.0"') && releaseWorkflow.includes('MACOSX_DEPLOYMENT_TARGET: "12.0"'), "Mac pipelines share an explicit deployment target");
...
ok(fastWorkflow.includes("check-macos-min-version.mjs") && releaseWorkflow.includes("check-macos-min-version.mjs"), "Fast and Universal Mac pipelines reject runtimes requiring a newer macOS than supported");
ok(releaseWorkflow.includes("macos-15-intel") && releaseWorkflow.includes("macos-15"), "Universal release builds native Bot API halves on Intel and ARM runners");
ok(releaseWorkflow.includes("lipo -create") && releaseWorkflow.includes("telegram-bot-api"), "Universal release combines Bot API ARM64+x86_64 with lipo");
ok(releaseWorkflow.includes("APP_MAIN_PATH") && releaseWorkflow.includes("BOT_API_PATH") && releaseWorkflow.includes("NODE_PATH") && releaseWorkflow.includes("FFMPEG_PATH"), "final DMG verifies app + Node + FFmpeg + Bot API architectures");
ok(!releaseWorkflow.includes('APPLE_SIGNING_IDENTITY: "-"'), "public Universal release is not ad-hoc signed");
ok(releaseWorkflow.includes("xcrun stapler validate") && releaseWorkflow.includes("codesign --verify"), "public Mac release verifies signing/notarization output");
ok(releaseWorkflow.includes("Developer ID sign embedded Universal Mach-O runtimes") && releaseWorkflow.includes("codesign --force --timestamp --options runtime --sign"), "Universal release Developer-ID signs every embedded executable before bundling");
ok(releaseWorkflow.includes('codesign -dv --verbose=4 "$path"') && releaseWorkflow.includes('Authority=Developer ID Application:'), "final Universal DMG verifies Developer ID authority on nested Mach-O executables");
ok(fastWorkflow.includes("Ad-hoc sign embedded Mach-O runtimes for test bundle") && fastWorkflow.includes('codesign --force --sign - "$path"'), "Fast ARM64 test bundle signs embedded executables for Apple Silicon");
ok(releaseWorkflow.includes("darwin-aarch64") && releaseWorkflow.includes("darwin-x86_64") && releaseWorkflow.includes(".app.tar.gz.sig"), "Mac updater publishes signed updater entries for both machine architectures");
ok(releaseWorkflow.includes("BEATGALER_UPDATER_ENDPOINT: https://github.com/magosouljah/galer/releases/latest/download/latest.json"), "Mac release compiles the real HTTPS updater endpoint into the app");
...
ok(fastWorkflow.includes("cargo test --locked --manifest-path src-tauri/Cargo.toml --lib") && releaseWorkflow.includes("cargo test --locked --manifest-path src-tauri/Cargo.toml --lib"), "Mac build pipelines refuse dependency resolution drift");
ok(fastWorkflow.includes("cargo metadata --locked") && releaseWorkflow.includes("cargo metadata --locked") && windowsUpdater.includes("cargo metadata --locked"), "public Desktop builds preflight the committed Rust dependency graph");
...
ok((fastWorkflow.match(/check-macos-runtime-dylibs\.sh/g) || []).length >= 2 && (releaseWorkflow.match(/check-macos-runtime-dylibs\.sh/g) || []).length >= 2, "Fast and Universal pipelines audit FFmpeg, Node, and Bot API both before bundling and inside the DMG");

ok(windowsUpdater.includes("beatgaler-public-release-") && releaseWorkflow.includes("beatgaler-public-release-"), "Windows and Mac public releases share a concurrency guard");
ok(windowsUpdater.includes("--merge-existing") && windowsUpdater.includes("gh release download"), "Windows updater preserves already-published Mac platform entries");
...
ok(fastWorkflow.indexOf("Prepare arm64 FFmpeg and Node runtime") < fastWorkflow.indexOf("cargo test --locked --manifest-path src-tauri/Cargo.toml --lib"), "Fast Mac Rust gate compiles only after required bundle resources exist");
ok(releaseWorkflow.indexOf("Combine pinned Bot API into Universal runtime") < releaseWorkflow.indexOf("cargo test --locked --manifest-path src-tauri/Cargo.toml --lib"), "Universal Mac Rust gate compiles only after required bundle resources exist");
