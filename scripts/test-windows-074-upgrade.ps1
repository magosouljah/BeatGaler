...
try {
  ...
  $legacyExe = Join-Path $legacyInstallDir "beat-galer.exe"
  ...
  $installedMain = Get-ChildItem -LiteralPath $legacyInstallDir -Filter *.exe -File | Where-Object { $_.Name -ne "uninstall.exe" } | Select-Object -First 1
  Assert-True ($null -ne $installedMain) "Galer main executable was not written into the legacy installation directory."
  ...
  Write-Host "PASS Windows 0.7.4 -> Galer upgrade: install location, registrations and shortcut choices preserved in-place."
}
finally {
  Remove-PathIfPresent $legacyProductKey
  Remove-PathIfPresent $legacyUninstallKey
