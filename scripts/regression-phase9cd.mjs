...
if (!projects.includes('inspectProjectDropSource(filePath)')) fail("PROJECT auto-inspection disappeared.");
if (!projects.includes('startProjectAssetUpdate(beat, filePath, "projectFile")')) fail("PROJECT file update route disappeared.");
if (!app.includes('startProjectAssetUpdate(beat, filePath, "projectFolder")') || !projects.includes('kind: "projectFile" | "projectFolder"')) fail("PROJECT folder/Samples update route disappeared.");
...
if (!projects.includes('libraryStateManager.commitSnapshot(beatsLatestRef.current, "project-sync")')) fail("PROJECT updates lost an authoritative commit.");
