...
fn build_cloud_manifest_entry(
    ...
    let project = match project_row {
        Ok((raw, source_size)) => {
            ...
            // Telegram's library index is the source of truth for PROJECT
            // capabilities. A PROJECT may contain only Audio/Samples, so the
            // existence of a PROJECT ZIP must never imply that it is openable.
            if let Some(obj) = project_manifest.as_object_mut() {
                obj.insert("openable".to_string(), Value::Bool(beat.has_flp));
                obj.insert("has_flp".to_string(), Value::Bool(beat.has_flp));
                obj.insert("has_als".to_string(), Value::Bool(beat.has_als));
            ...
            Some(json!({
                ...
                "has_flp": beat.has_flp,
                "has_als": beat.has_als,
                "has_samples": beat.has_samples,
            }))
        }
...
pub fn sync_cloud_library_index(
    ...
    let mut args = vec![
        ...
        "POST".to_string(),
        "-F".to_string(),
        format!("beatgalerUserId={}", user_id),
        "-F".to_string(),
    ...
    Ok(CloudLibrarySyncResult {
        telegram_file_id: response.get("telegram_file_id")
            .and_then(|v| v.as_str()).unwrap_or("").to_string(),
        telegram_message_id: response.get("telegram_message_id")
...
fn restore_cloud_records_from_manifest_entry(
    ...
    if let Some(project) = entry.get("project").filter(|v| !v.is_null()) {
        ...
        if let Some(obj) = project_manifest_value.as_object_mut() {
            for key in ["openable", "has_flp", "has_als", "has_samples"] {
                if let Some(value) = project.get(key).cloned() {
...
pub fn upload_dropped_file_to_telegram(
    ...
    if cloud_type == "PROJECT" && !project_zip_is_valid(&source) {
        return Err("PROJECT zip is invalid. It must contain FLP/ALS or Audio/Sample/Samples and no Backup/Backups.".to_string());
    }
...
/// to PROJECT: FLP/ALS plus Audio/Sample/Samples. Backup/Backups is forbidden.
...
fn build_project_archive_if_needed(
    ...
    if !folder_has_project_assets(&folder) {
        return Err("No PROJECT assets were found. Add an FLP/ALS, Audio/, Sample/, or Samples/ folder.".to_string());
    }
    ...
    if !folder_has_project_assets(&staging) {
        let _ = std::fs::remove_dir_all(&staging);
        return Err("PROJECT staging contains no FLP/ALS, Audio/, Sample/, or Samples/ assets.".to_string());
    }
...
fn folder_has_project_assets(folder: &Path) -> bool {
    ...
    for entry in rd.flatten() {
        ...
        if path.is_dir() {
            ...
            if matches!(name.to_ascii_lowercase().as_str(), "audio" | "sample" | "samples") {
                return true;
            }
            continue;
        }
...
fn project_zip_is_valid(path: &Path) -> bool {
    if !path.is_file()
        || path.extension().and_then(|v| v.to_str()).unwrap_or("").to_ascii_lowercase() != "zip"
    {
        return false;
    }
...
try {
  $hasProjectAsset = $false
  $hasForbiddenBackup = $false
  foreach ($e in $z.Entries) {
    ...
    foreach ($part in $parts) {
      ...
      if ($lowerPart -eq 'backup' -or $lowerPart -eq 'backups') {
        $hasForbiddenBackup = $true
        break
      }
    }
    if ($hasForbiddenBackup) { break }

    $rootName = $parts[0].ToLowerInvariant()
    if ($parts.Count -eq 1) {
      $ext = [System.IO.Path]::GetExtension($rootName).ToLowerInvariant()
      if ($ext -eq '.flp' -or $ext -eq '.als') { $hasProjectAsset = $true }
    } elseif ($rootName -eq 'audio' -or $rootName -eq 'sample' -or $rootName -eq 'samples') {
      $hasProjectAsset = $true
    }
  }
  if ($hasProjectAsset -and -not $hasForbiddenBackup) { Write-Output '1' } else { Write-Output '0' }
} finally {
  $z.Dispose()
}
"#;
        ...
        return Command::new("unzip")
            ...
            .map(|out| {
                ...
                for raw in String::from_utf8_lossy(&out.stdout).lines() {
                    ...
                    if parts.iter().any(|p| is_forbidden_project_component(p)) {
                        return false;
                    }
                    if parts.len() == 1 {
                        let lower = parts[0].to_ascii_lowercase();
                        if lower.ends_with(".flp") || lower.ends_with(".als") {
                            has_asset = true;
                        }
                    } else {
                        let root = parts[0].to_ascii_lowercase();
                        if matches!(root.as_str(), "audio" | "sample" | "samples") {
                            has_asset = true;
                        }
                    }
                }
...
fn ensure_project_working_copy(
    ...
    if let Some((saved_path, manifest, _, _)) = project_manifest(conn, &beat.id)? {
        ...
        if !project_zip_is_valid(&workspace) {
            ...
            return Err("The PROJECT zip is invalid. It must contain FLP/ALS or Audio/Sample/Samples and no Backup/Backups.".to_string());
        }
...
fn ensure_unique_beat_name(conn: &Connection, beat: &BeatMeta) -> rusqlite::Result<()> {
    let wanted = normalized_beat_name_key(&beat.name);
    if wanted.is_empty() {
    ...
    let mut stmt = conn.prepare("SELECT id, meta_json FROM beats WHERE id<>?1")?;
    let rows = stmt.query_map(params![beat.id.clone()], |row| {
    ...
    for row in rows {
        ...
        let Ok(existing) = serde_json::from_str::<BeatMeta>(&raw) else { continue; };
        if normalized_beat_name_key(&existing.name) == wanted {
            return Err(rusqlite::Error::InvalidParameterName(format!(
                "A beat named '{}' already exists. Beat names must be unique.",
                beat.name
            )));
        }
    }

    // Trash reserves the name too. Otherwise a new active beat could take the
    // same name and make the original impossible to restore without conflict.
    let mut trash_stmt = conn.prepare("SELECT beat_name FROM trash")?;
    let trash_names = trash_stmt.query_map([], |row| row.get::<_, String>(0))?;
    for name in trash_names {
        if normalized_beat_name_key(&name?) == wanted {
            return Err(rusqlite::Error::InvalidParameterName(format!(
                "A beat named '{}' already exists in Trash. Restore or permanently remove it before reusing the name.",
                beat.name
            )));
        }
    }
...
pub fn purge_old_trash_internal(conn: &Connection, data_dir: &Path, max_age_days: i64) -> usize {
    ...
    // For Cloud trash, Telegram deletion is authoritative. Never erase the
    // local Trash row first, otherwise a temporary network failure creates an
    // orphaned Topic in Telegram.
    let settings_snapshot = load_settings(data_dir);
    let cloud_user_id = settings_snapshot.beatgaler_user_id.clone();
    ...
    for (trash_id, path, beat_meta_json, is_cloud) in rows {
        if is_cloud {
            ...
            let Some(user_id) = cloud_user_id.as_deref() else {
                log_warn(data_dir, &format!(
                    "Skipped permanent Cloud trash delete for '{}' because BeatGaler user id is unavailable",
                    meta.name
                ));
                continue;
            };
            ...
            let base = telegram_cloud_api_base();
            let url = format!("{}/beats/delete-topic", base);
            let body = serde_json::json!({
                "beatgalerUserId": user_id,
                "beatId": meta.id,
            });

            if let Err(err) = post_json_simple(&url, &body) {
                log_warn(data_dir, &format!(
                    "Skipped local purge for Cloud beat '{}' because Telegram topic delete failed: {}",
                    meta.name, err
                ));
                continue;
            }
    ...
    if purged > 0 {
        log_info(data_dir, &format!(
            "Purged {} trash item(s) older than {} days; Cloud Topics were deleted first",
            purged, max_age_days
        ));
    }
    purged
...
pub fn list_openable_cloud_project_beat_ids(
    ...
    for row in rows {
        ...
        let openable = manifest.get("openable").and_then(|v| v.as_bool())
            .or_else(|| manifest.get("has_flp").and_then(|v| v.as_bool()))
            .unwrap_or(false);
        if openable { out.push(beat_id); }
    }
    Ok(out)
}
...
pub fn resolve_import_decisions(
    ...
    for (core_name, group) in batch.groups {
        ...
        let base_display_name = titleize(&core_name);
