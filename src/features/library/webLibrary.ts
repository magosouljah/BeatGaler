...
export interface WebLibraryLoadOptions { online?: boolean; queryActive?: boolean; now?: () => number; onObservation?: (observation: WebLibraryLoadObservation) => void; pageSize?: number; }
...
function validateWebLibrarySource(value: unknown): JsonRecord { const source = record(value); if (!source || source.schema !== GALER_T_LIBRARY_SCHEMA) throw new Error("Galer Cloud returned an invalid library index."); const version = Number(source.version ?? 1); if (!Number.isInteger(version) || version < 1) throw new Error("Galer Cloud returned an unsupported library index."); if (version > GALER_T_LIBRARY_VERSION) throw new Error("This library requires a newer BeatGaler version."); return source; }
function deletedBeatIds(source: JsonRecord): Set<string> { const deleted = Array.isArray(source.deleted) ? source.deleted : []; return new Set(deleted.map(record).map(row => text(row?.beat_id ?? row?.id).trim()).filter(Boolean)); }
export function normalizeWebLibraryManifest(value: unknown): WebLibraryManifest { const source = validateWebLibrarySource(value); const deleted = Array.isArray(source.deleted) ? source.deleted : []; const deletedIds = deletedBeatIds(source); const beats = (Array.isArray(source.beats) ? source.beats.map(record).filter((beat): beat is JsonRecord => beat !== null) : []).filter(beat => !deletedIds.has(text(beat.id).trim())); const trash = (Array.isArray(source.trash) ? source.trash : []).filter(value => { const row = record(value); const beat = record(row?.beat) || row; return !deletedIds.has(text(beat?.id).trim()); }); return { schema: GALER_T_LIBRARY_SCHEMA, version: GALER_T_LIBRARY_VERSION, beats, trash, deleted, updated_at: Number.isFinite(Number(source.updated_at)) ? Number(source.updated_at) : undefined }; }
...
function boundedPageFromManifest(value: unknown, offset: number, pageSize: number): WebLibraryPage {
  ...
  for (const value of rawBeats) {
    ...
    if (!entry || deletedIds.has(text(entry.id).trim())) continue;
    if (visibleIndex >= normalizedOffset && beats.length < normalizedPageSize) {
      beats.push(beatFromWebLibraryEntry(entry));
    }
    visibleIndex += 1;
    totalVisible += 1;
  }
...
// Page loading scans the compact INDEX to locate the requested window, but only converts that
// window into rich Beat objects. This makes resident Beat[] memory bounded by pageSize instead
// of library cardinality while preserving the authoritative single-index contract.
export async function loadWebLibraryPage(transport: WebLibraryTransport, options: WebLibraryPageOptions = {}): Promise<WebLibraryPage> {
  const now = options.now || (() => typeof performance !== "undefined" ? performance.now() : Date.now());
  const startedAt = now();
  try {
    const index = await getOrBootstrapLibraryIndex(transport);
    const page = boundedPageFromManifest(index.manifest, options.offset ?? 0, options.pageSize ?? WEB_LIBRARY_FIRST_PAGE_SIZE);
    options.onObservation?.({ state: classifyWebLibraryResult(page.totalVisible, options.queryActive), durationMs: Math.max(0, now() - startedAt), beatCount: page.beats.length });
    return page;
  } catch (error) {
...
// Initial library load intentionally returns metadata only and only the first bounded page.
// Artwork remains represented by assets.artwork and is resolved on demand by the presentation/
// media path instead of blocking startup on an N-artwork download batch. Call loadWebLibraryPage
// with nextOffset to continue incrementally without ever materializing the complete Beat[] at once.
export async function loadWebLibrary(transport: WebLibraryTransport, options: WebLibraryLoadOptions = {}): Promise<Beat[]> {
  const page = await loadWebLibraryPage(transport, { ...options, offset: 0, pageSize: options.pageSize ?? WEB_LIBRARY_FIRST_PAGE_SIZE });
  return page.beats;
}
