...
export default function SettingsPanel(props: Props) {
  ...
  useEffect(() => {
    ...
    getPlaybackCacheStatus().then(status => { setCacheStatus(status); setCustomCacheMb(String(status.limit_mb)); }).catch(console.error);
  }, []);
  ...
  const updateCacheLimit = async (limitMb: number) => {
    setCacheBusy(true);
    try {
      const status = await setPlaybackCacheLimitMb(Math.max(0, Math.round(limitMb)));
  ...
  const clearCache = async () => {
    setCacheBusy(true);
    try { setCacheStatus(await clearPlaybackCache()); }
    finally { setCacheBusy(false); }
  };
  ...
  const emptyTrash = () => {
    ...
    void (async () => {
      try {
        const purged = await purgeTrashNow();
        ...
        setTrashError(String(e?.message || e));
        setTrashMessage(null);
        try { setTrashItems(await listTrash()); } catch {}
      } finally {
        setPurging(false);
      }
    })();
  ...
  const emptyPresetTrash = async () => {
    ...
    setPurgingPresets(true); try { await purgeTemplateTrashNow(); setPresetTrashItems([]); } finally { setPurgingPresets(false); }
  };
    ...
    <main style={{ flex: 1, overflowY: "auto" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "38px 34px 80px" }}>
        {active === "account" && <>
          ...
          {accountMessage && <div style={{ marginBottom: 14, color: "#83a783", fontSize: 11 }}>{accountMessage}</div>}{accountError && <div style={{ marginBottom: 14, padding: 10, border: "1px solid #542020", background: "#241010", borderRadius: 8, color: "#e6a0a0", fontSize: 11 }}>{accountError}</div>}
        ...
        {active === "plan" && <>
          ...
          {accountError && <div style={{ marginTop: 12, padding: 10, border: "1px solid #542020", background: "#241010", borderRadius: 8, color: "#e6a0a0", fontSize: 10 }}>{accountError}</div>}
        </>}
