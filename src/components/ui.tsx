...
import { cleanTags, normalizeTag, validateTag } from "../lib/metadataValidation";
import { getProceduralBeatBackgroundDataUrl } from "../lib/proceduralBeatBackground";
    ...
    <span style={{
      ...
      {onRemove && (
        <button
          ...
          style={{
            margin: "-2px -4px -2px 1px", padding: "0 3px", border: 0, background: "transparent",
            color: color ?? "#666", opacity: color ? 0.82 : 1, cursor: "pointer", fontSize: 13, lineHeight: 1,
          }}
        >×</button>
      )}
    </span>
...
export function TagEditor({
  ...
  useEffect(() => {
    ...
    const same = cleaned.tags.length === tags.length && cleaned.tags.every((tag, i) => tag === normalizeTag(tags[i] || ""));
    if (!same) onChange(cleaned.tags);
  }, [tags, onChange]);

  const inputValidation = input.trim() ? validateTag(input) : null;
  const inputInvalid = !!inputValidation && !inputValidation.valid;

  const pushUniqueTag = (nextTag: string): boolean => {
    const result = validateTag(nextTag);
    if (result.valid === false) {
      setTagError(`Invalid tag — ${result.reason}`);
      return false;
    }
    setTagError(null);
    const existing = new Set(tags.map(normalizeTag));
    if (existing.has(result.normalized)) return true;
    if (tags.length >= 30) {
      setTagError("Invalid tag — Maximum 30 tags per beat.");
      return false;
    }
    onChange([...tags.map(normalizeTag).filter(Boolean), result.normalized]);
    return true;
  };

  const normalizedInput = normalizeTag(input);
  const add = () => {
    if (pushUniqueTag(input)) setInput("");
  };

  const predictedTags = suggestions
    .map(normalizeTag)
    .filter((s, i, arr) => !!s && arr.indexOf(s) === i)
    .filter((s) => validateTag(s).valid)
    .filter((s) => !tags.map(normalizeTag).includes(s) && (!normalizedInput || s.includes(normalizedInput)))
    .slice(0, 5);

  const liveError = inputInvalid ? `Invalid tag — ${inputValidation.reason}` : tagError;
  ...
  return (
    <div data-prevent-enter-save="true">
          ...
          {predictedTags.map((t) => (
            <button
              ...
              style={{
                ...
                background: "#121212", color: "#8d8d8d", border: "1px solid #252525", cursor: "pointer",
              }}
            >
              + {t}
            </button>
      ...
      {liveError && <div style={{ fontSize: 10, color: "#f87171", marginBottom: 6 }}>{liveError}</div>}
        ...
        <input
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === "Tab" && predictedTags.length > 0) {
              e.preventDefault();
              e.stopPropagation();
              if (pushUniqueTag(predictedTags[0])) setInput("");
            }
          }}
          placeholder="Add tag, press Enter…"
          aria-invalid={inputInvalid}
          style={{
            flex: 1, background: inputInvalid ? "#1d1010" : "#161616",
            border: `1px solid ${inputInvalid ? "#7f1d1d" : "#222"}`, borderRadius: 7,
            padding: "6px 10px", color: inputInvalid ? "#fca5a5" : "#ccc", fontSize: 12, outline: "none",
          }}
        />
        <button
          type="button"
          onClick={add}
          style={{
            padding: "6px 12px", background: inputInvalid ? "#2a1111" : "#232323",
            border: `1px solid ${inputInvalid ? "#6b1b1b" : "#2e2e2e"}`,
            borderRadius: 7, color: inputInvalid ? "#f87171" : "#999", fontSize: 12, cursor: "pointer",
          }}
        >Add</button>
      </div>
    </div>
...
export function Artwork({
  ...
  const artworkSrc = beat.image_preview_base64 || beat.image_base64;
  const initials = beat.name.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase();
  const proceduralBackground = useMemo(
    () => (artworkSrc ? null : getProceduralBeatBackgroundDataUrl(beat.id)),
    [artworkSrc, beat.id],
  );
  const backgroundImage = artworkSrc
    ? undefined
    : (proceduralBackground ? `url(${proceduralBackground})` : `linear-gradient(135deg, ${beat.color}, ${beat.color2})`);
  return (
    <div style={{
      width: size, height: size, borderRadius: 10, flexShrink: 0,
      position: "relative", overflow: "hidden",
      backgroundColor: artworkSrc ? "#000" : "#0c0c0c",
      backgroundImage,
      ...
      {artworkSrc ? (
        <img src={artworkSrc ?? undefined} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt={beat.name} />
      ) : (
        <span style={{
