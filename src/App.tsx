...
function BeatGalerApp() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const shell = await platform.startup.loadAuthenticatedShell();
        if (cancelled) return;
        if (shell) {
          beatsLatestRef.current = shell.beats;
          setSettings(shell.settings);
          setBeats(shell.beats);
          setConnectionState(shell.connectionState);
          setCloudSessionVerified(shell.libraryVerified);
          setSetupDone(true);
          setLoading(false);
          startupCookingResolvedRef.current = true;
          startupPipelineStartedRef.current = false;
          progressiveRevealRunRef.current += 1;
          setRevealedBeatIds(new Set(shell.beats.map(beat => beat.id)));
          setStartupCookingGate(false);
          dismissBeatGalerStartupLoader();
          return;
        }
        ...
        const local = await platform.preferences.load();
        if (cancelled) return;
        setSettings(local);
        setSetupDone(true);
        setLoading(false);
        ...
        let status: Awaited<ReturnType<typeof platform.cloud.status>> = { connected: false, reachable: false, username: null };
        try {
          status = await platform.cloud.status();
          if (cancelled) return;
          ...
          if (local.telegram_cloud_connected && !(status.connected && status.reachable)) {
            for (let attempt = 1; attempt <= 12; attempt += 1) {
              ...
              if (cancelled) return;
              status = await platform.cloud.status().catch(() => status);
        ...
        const flushedTrashCount = await platform.library.flushOfflineTrashIntents();
  ...
  useEffect(() => {
    ...
    const reconnect = async () => {
      ...
      for (const delay of delays) {
        ...
        try {
          const status = await platform.cloud.status();
          if (disposed || run !== networkReconnectRunRef.current) return;
          if (!status.reachable) {
  ...
  useEffect(() => {
    ...
    const sourceId = platform.clientId;
  ...
  const hydrateWebReviewCandidate = useCallback((hydrated: Beat) => {
    setReviewQueue(queue => {
      if (!queue || !queue.beats.some(beat => beat.id === hydrated.id)) return queue;
      return {
        ...queue,
        beats: queue.beats.map(current => current.id === hydrated.id ? {
          ...current,
          bpm: current.bpm || hydrated.bpm,
          key: current.key || hydrated.key,
          tags: current.tags.length > 0 ? current.tags : hydrated.tags,
          image_base64: current.image_base64 || hydrated.image_base64,
        } : current),
      };
    });
  }, []);
  ...
  const handleReviewedSaveAll = useCallback(async (currentUpdated: Beat) => {
    ...
    if (platform.capabilities.directGalerCloudTransport) cloudifyImportedBeats([currentUpdated]);
  ...
  const handleDroppedBeatFileRole = useCallback(async (role: DroppedBeatFileRole) => {
    ...
    if (role === "main") {
      ...
      runBeatCloudUpdate(beat, filePath, async () => {
        ...
        const refreshed = await platform.library.load();
  ...
  useEffect(() => {
    ...
    return installHtmlDropController({
      ...
      onBrowserLibraryFileDrop: platform.capabilities.singleBeatDrop ? async files => {
        if (files.length !== 1) {
          await appAlert({
            title: "Import one beat at a time",
            message: "BeatGaler Web accepts exactly one MP3 or WAV per picker or drop action.",
          });
          return;
        }
        const candidate = platform.importer.fromFile(files[0]);
        addBeatsAndReview([candidate.beat]);
        void candidate.hydrated.then(hydrateWebReviewCandidate).catch(() => {});
      } : undefined,
...
{tagRename && (() => {
  ...
  return ReactDOM.createPortal(
    <>
      ...
      {/* + Add beat */}
      {!currentBeat && (platform.capabilities.browserFileImport || platform.capabilities.nativeFilesystemDrop) && (
      ...
      {showAdd && (platform.capabilities.browserFileImport || platform.capabilities.nativeFilesystemDrop) && <AddBeatModal onClose={() => setShowAdd(false)} onAdd={addBeatsAndReview} onCandidateHydrated={hydrateWebReviewCandidate} existingBeats={beats} />}
