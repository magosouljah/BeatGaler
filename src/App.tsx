...
function BeatGalerApp() {
  ...
  const recoverTelegramLibraryOnceIfEmpty = useCallback(async () => {
    ...
    try {
      const cloud = await import("./lib/tauri");
      const restored = await cloud.restoreLibraryFromTelegram();
  ...
  useEffect(() => {
    if (!settings?.telegram_cloud_connected) return;
    if (beats.length !== 0) return;
    void recoverTelegramLibraryOnceIfEmpty();
  }, [settings?.telegram_cloud_connected, beats.length, recoverTelegramLibraryOnceIfEmpty]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("beatgaler:project-cloud-changed", refresh);
              ...
              <div style={{ display: "flex", justifyContent: "center", width: "100%" }}>
                ...
                {filteredBeats.map((beat, i) => (
                  <BeatCard
                    key={beat.id}
                    beat={beat}
                    openableProject={openableCloudProjectIds.has(beat.id)}
