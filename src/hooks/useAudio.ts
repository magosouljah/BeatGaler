...
export function useAudio() {
  ...
  const getAudio = useCallback(() => {
    if (!audioRef.current) { audioRef.current = new Audio(); audioRef.current.volume = 0.75; }
  ...
  useEffect(() => {
    ...
    const onPlay = () => { waitingRef.current = false; playTrace("AUDIO_EVENT_PLAY", { beat_id: currentBeatIdRef.current, ready_state: audio.readyState }); setState(s => ({ ...s, isPlaying: true })); publishPlaybackState(); };
    const onPlaying = () => { const beatId = currentBeatIdRef.current; waitingRef.current = false; playTrace("AUDIO_EVENT_PLAYING", { beat_id: beatId, ready_state: audio.readyState, current_time: audio.currentTime }); setState(s => ({ ...s, isPlaying: true })); publishPlaybackState(); if (beatId) window.dispatchEvent(new CustomEvent("beatgaler:audio-playing", { detail: { beatId } })); void platform.diagnostics.audioEvent("AUDIO_PLAYING", beatId, null, `readyState=${audio.readyState} currentTime=${audio.currentTime.toFixed(3)}`).catch(() => {}); };
    const onWaiting = () => { waitingRef.current = true; playTrace("AUDIO_EVENT_WAITING", { beat_id: currentBeatIdRef.current, ready_state: audio.readyState, current_time: audio.currentTime }); publishPlaybackState(); void platform.diagnostics.audioEvent("AUDIO_WAITING", currentBeatIdRef.current, null, `readyState=${audio.readyState} currentTime=${audio.currentTime.toFixed(3)}`).catch(() => {}); };
    const onCanPlay = () => { waitingRef.current = false; playTrace("AUDIO_EVENT_CANPLAY", { beat_id: currentBeatIdRef.current, ready_state: audio.readyState }); publishPlaybackState(); void platform.diagnostics.audioEvent("AUDIO_CANPLAY", currentBeatIdRef.current, null, `readyState=${audio.readyState}`).catch(() => {}); };
  ...
  const primeAudioEngine = useCallback(async (path: string): Promise<boolean> => {
    ...
    return await new Promise<boolean>(resolve => {
      ...
      const onPrimeError = () => { cleanup(); primingRef.current = false; audio.muted = false; void platform.diagnostics.audioEvent("AUDIO_ENGINE_PRIME_ERROR", null, null, "").catch(() => {}); resolve(false); };
      audio.addEventListener("canplay", onReady, { once: true }); audio.addEventListener("error", onPrimeError, { once: true }); audio.src = source; audio.currentTime = 0; audio.load();
    });
  }, [getAudio]);
  ...
  const play = useCallback((beatId: string, paths: string[]) => {
    const audio = getAudio(); playTrace("AUDIO_PLAY_FUNCTION_ENTER", { beat_id: beatId, ready_state: audio.readyState, paused: audio.paused }); const sources = Array.from(new Set(paths.filter(Boolean).map(path => platform.media.resolveUrl(path))));
    if (!shouldAcceptWebPlaybackRequest(beatId, sources)) { playTrace("AUDIO_SUPERSEDED_PLAY_IGNORED", { beat_id: beatId }); return; }
    if (sources.length === 0) { console.error("No audio sources available for beat", beatId); return; }
