...
export interface AppSettings {
  ...
  playback_cache_limit_mb?: number;
}

export interface TelegramCloudStatus {
  // `connected` means the BeatGaler account is linked to Telegram.
  // `reachable` is runtime transport health and may be false while the
  // remembered link remains valid (for example when Wi-Fi is off).
  connected: boolean;
  reachable: boolean;
