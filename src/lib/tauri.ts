...
export async function loadOfflineLibrary(): Promise<Beat[]> {
  await initTauri();
  if (!invoke) return [];
  return invoke<Beat[]>("load_offline_library");
}

export async function makeBeatAvailableOffline(beat: Beat): Promise<Beat> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<Beat>("make_beat_available_offline", { beat });
}

export async function removeBeatOfflineAvailability(beatId: string): Promise<void> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<void>("remove_beat_offline_availability", { beatId });
}

export async function recordOfflineTrashIntent(beatId: string): Promise<void> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<void>("record_offline_trash_intent", { beatId });
}
...
export async function purgeInterruptedUploadLocal(beatId: string, stagingPaths: string[]): Promise<void> {
  await initTauri();
  if (!invoke) return;
  return invoke<void>("purge_interrupted_upload_local", { beatId, stagingPaths });
}
...
export function filePathToUrl(filePath: string): string {
  // Remote/media URLs must stay untouched even inside Tauri. Passing an HTTP
  // stream URL through convertFileSrc() would turn it into an asset URL and
  // break progressive playback / Range requests.
...
export async function getPlaybackCacheStatus(): Promise<PlaybackCacheStatus> {
  await initTauri();
  if (!invoke) return { used_bytes: 0, limit_mb: 2048 };
  return invoke<PlaybackCacheStatus>("get_playback_cache_status");
}

export async function setPlaybackCacheLimitMb(limitMb: number): Promise<PlaybackCacheStatus> {
  await initTauri();
  if (!invoke) return { used_bytes: 0, limit_mb: limitMb };
  return invoke<PlaybackCacheStatus>("set_playback_cache_limit_mb", { limitMb });
}

export async function clearPlaybackCache(): Promise<PlaybackCacheStatus> {
  await initTauri();
  if (!invoke) return { used_bytes: 0, limit_mb: 2048 };
  const status = await invoke<PlaybackCacheStatus>("clear_playback_cache");
  // The Rust cooker and the WebView Fast Play memo are separate runtimes.
  // Clearing disk/native cooker state must invalidate the WebView memo too,
  // otherwise the first Play can reuse a dead localhost URL and fail once.
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("beatgaler:playback-cache-cleared"));
  }
  return status;
}
...
export async function connectTelegramCloud(): Promise<void> {
  ...
  // Open Telegram. Only while this login attempt is pending, check the small
  // connection-status endpoint. This is NOT library synchronization and is not
  // a permanent timer.
  await invoke<void>("connect_telegram_cloud");

  const invokeNow = invoke;
  void (async () => {
    ...
    while (Date.now() < deadline) {
      ...
      try {
        const status = await invokeNow<TelegramCloudStatus>("poll_telegram_cloud_status");
        if (!status.connected) continue;

        // Do NOT reload the WebView. Reloading destroys the live React library
        // state and was the regression introduced by the previous hotfix.
        if (typeof window !== "undefined") {
          window.dispatchEvent(new CustomEvent("beatgaler:telegram-connected", {
            detail: status,
          }));
        }
        return;
      } catch {
        // Temporary handoff/network miss: retry only while this login is pending.
      }
    }
  })();
}
...
export async function copyAudioMetadata(
  sourcePath: string,
  destinationPath: string,
): Promise<string> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<string>("copy_audio_metadata", { sourcePath, destinationPath });
}
...
export type BackgroundDownloadKind = "MP3" | "WAV" | "PROJECT" | "ALL";

export interface BackgroundDownloadEvent {
  task_id: string;
...
export async function startBackgroundDownload(
  ...
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<string>("start_background_download", {
    kind,
    beat,
    destinationPath,
  });
...
// Cloud-first playback: cached MASTERs use the private cache; otherwise Rust
// returns a Range-capable cloud stream URL immediately.
...
export async function warmBeatForPlayback(beat: Beat): Promise<string | null> {
  await initTauri();
  if (!invoke || !beat.telegram_file_id) return null;
  return invoke<string | null>("warm_beat_for_playback", { beat });
}

export async function prefetchBeatForPlayback(beat: Beat): Promise<void> {
  await warmBeatForPlayback(beat);
}

export interface DownloadCookingEntryStatus {
  beat_id: string;
  beat_name: string;
...
export interface DownloadCookingStatus {
  ready_bytes: number;
  diagnostic_path: string;
  entries: DownloadCookingEntryStatus[];
}

export async function getDownloadCookingStatus(): Promise<DownloadCookingStatus> {
  await initTauri();
  if (!invoke) return { ready_bytes: 512 * 1024, diagnostic_path: "", entries: [] };
  return invoke<DownloadCookingStatus>("get_download_cooking_status");
}

export async function downloadCookingDiagnosticEvent(
  event: string,
  beatId?: string | null,
  ...
  await initTauri();
  if (!invoke) return "";
  return invoke<string>("download_cooking_diagnostic_event", {
    event,
    beatId: beatId ?? null,
...
export interface ProjectDropInspection {
  kind: "zip" | "project_file" | "folder" | "unsupported";
  valid: boolean;
  reason: string | null;
  has_backups: boolean;
...
export async function inspectProjectDropSource(sourcePath: string): Promise<ProjectDropInspection> {
  await initTauri();
  if (!invoke) return { kind: "unsupported", valid: false, reason: "Tauri not available", has_backups: false, project_file_count: 0, entry_count: 0 };
  return invoke<ProjectDropInspection>("inspect_project_drop_source", { sourcePath });
}
...
export async function uploadProjectToTelegram(beat: Beat): Promise<ProjectCloudStatus> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  const result = await invoke<ProjectCloudStatus>("upload_project_to_telegram", { beat });
  if (typeof window !== "undefined") window.dispatchEvent(new Event("beatgaler:project-cloud-changed"));
  return result;
}
...
export async function downloadProjectToCache(beat: Beat): Promise<string> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<string>("download_project_to_cache", { beat });
}

export type ProjectAssetKind = "flp" | "samples" | "audio" | "projectFile" | "projectFolder";
...
export interface BeatFolderUpdatePreview {
  has_mp3: boolean;
...
export interface ImportAudioConflict {
  core_name: string;
  display_name: string;
  kind: "master_mp3" | "source_wav" | "hq_wav" | "main_audio";
  candidates: string[];
}
...
export interface ImportBatchPreview {
  ...
  audio_conflicts: ImportAudioConflict[];
}

export interface ImportReviewStep {
  beat: Beat | null;
  prepared_count: number;
  total_normal: number | null;
  remaining_normal: number | null;
  discovery_complete: boolean;
}

export interface ImportReviewStreamStart {
  batch_id: string;
}
...
// Starts a zero-scan Review session. The filesystem is advanced lazily by
// prepareNextImportReviewBeat so Beat 1 never waits for the full batch.
export async function startImportReviewStream(rootPaths: string[]): Promise<ImportReviewStreamStart> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<ImportReviewStreamStart>("start_import_review_stream", { rootPaths });
}

export async function getImportReviewBatchSummary(batchId: string): Promise<ImportBatchPreview> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<ImportBatchPreview>("get_import_review_batch_summary", { batchId });
}
...
// Sequential Review preparation: one beat per invoke. This deliberately keeps
// ID3/artwork work for Beat 2..N out of Beat 1's critical path.
export async function prepareNextImportReviewBeat(batchId: string): Promise<ImportReviewStep> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<ImportReviewStep>("prepare_next_import_review_beat", { batchId });
}

export async function resolveImportAudioConflict(
  batchId: string,
  coreName: string,
  selectedPath: string,
): Promise<Beat> {
  await initTauri();
  if (!invoke) throw new Error("Tauri not available");
  return invoke<Beat>("resolve_import_audio_conflict", { batchId, coreName, selectedPath });
}

export async function discardImportReviewBatch(batchId: string): Promise<void> {
  await initTauri();
  if (!invoke) return;
  await invoke<void>("discard_import_review_batch", { batchId });
}
...
export async function repairStaleCloudLibraryRefs(): Promise<number> {
  await initTauri();
  if (!invoke) return 0;
  return invoke<number>("repair_stale_cloud_library_refs");
}
...
export async function loadCloudArtworkForBeat(beatId: string): Promise<string | null> {
  await initTauri();
  if (!invoke) return null;
  return invoke<string | null>("load_cloud_artwork_for_beat", { beatId });
}
