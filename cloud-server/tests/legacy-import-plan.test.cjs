...
const authRaw = JSON.stringify({
  users: [
    ...
    { id: 'u1', email: 'A@Example.com', mfaSecret: 'legacy', storageChatId: '-100', planState: { basePlanId: 'free', grants: [{ planId: 'paid_entry' }] }, providers: [{ provider: 'google' }] },
  ],
  sessions: { abc: { userId: 'u1' }, def: { userId: 'u2' } },
});
const dataRaw = JSON.stringify({
  linkedAccounts: { u1: { storageChatId: '-100' }, u2: { storageChatId: '-200' } },
  uploadedFiles: [],
  beatTopics: { 'u1:b1': 11 },
  pendingTopicDeletes: {},
  messageRedirects: {},
...
assert.throws(() => planLegacyImport({ users: [{ id: 'u1' }], sessions: {} }, { linkedAccounts: { missing: {} } }), /unknown users/);

console.log('PASS legacy import planner: deterministic, hashed, fail-closed');
